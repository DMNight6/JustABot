    ...
    run: async(client, interaction) => {
        ...
        if (string && !player.paused) {
            ...
            if (!client.TimerCFG[interaction.guild?.id!]) {
                player.pause(true);
                client.TimerCFG[interaction.guild?.id!] = setTimeout(() => { interaction.reply({ content: 'Unpaused player due to requested time reached.' }); player.pause(false) }, time);
            }
        ...
        if (player.paused) {
            ...
            if (client.TimerCFG[interaction.guild?.id!]) {
                ...
                // Cleanup.
                return interaction.reply({ content: 'Unpaused the player and canceled the unpause timer.' });
            };
            return interaction.reply({ content: 'Unpaused the player.' });
        };
    }
};
